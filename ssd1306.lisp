...
(defconstant OLED_PIXELBUF_LENGTH (+ 1 (* SSD1306_WIDTH (/ SSD1306_HEIGHT 8))))
;;;;;;;;;;;;;;;;
...
(defparameter *oled-wbuf*
    (cffi:foreign-alloc :uint8
                        :initial-contents
                        (zeros 3)))

(defparameter *oled-pixelbuf*
    (cffi:foreign-alloc :uint8
                        :initial-contents
                        (zeros OLED_PIXELBUF_LENGTH)))

(defparameter *oled-pixels* (make-array `(,(floor SSD1306_HEIGHT 8) ,SSD1306_WIDTH) :initial-element 0))
...
(defun set-oled-contrast (contrast)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #x81)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 2) contrast)
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 3))

(defun set-oled-display-on (on)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1)
          (logior #xae
                  (if on #x01 #x00)))
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 2))
...
(defun set-oled-deactivate-scroll ()
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #x2e)
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 2))

(defun set-oled-activate-scroll ()
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #x2f)
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 2))

(defun set-oled-memory-addressing-mode (mode)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #x20)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 2) mode)
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 3))
...
(defun set-oled-multiplex-ratio (ratio)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #xa8)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 2) ratio)
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 3))
...
(defun set-oled-display-offset (offset)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #xd3)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 2) offset)
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 3))

(defun set-oled-com-pins (alternative lr-remap)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #xda)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 2)
          (logior #x02
...
(defun set-oled-display-clock-div (div)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #xd5)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 2) div)
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 3))

(defun set-oled-pre-charge-period (period)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #xd9)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 2) period)
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 3))

(defun set-oled-v-comh-deselect-level (level)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #xdb)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 2)
          (ash level 4))
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 3))
...
(defun set-oled-charge-pump (enable)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) #x8d)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 2)
          (logior #x10
                  (if enable #x04 #x00)))
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 3))
...
(defun write-oled-pixels (page)
    ; PAGE ADDR
    (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
    (setf (cffi:mem-aref *oled-wbuf* :uint8 1) (logior #xb0 page))
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 2)
    ; COLUMN OFFSET
    (let ((x-offset MICRO_OLED_COLUMN_OFFSET)
        ...
        ; Top
        (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
        (setf (cffi:mem-aref *oled-wbuf* :uint8 1) (logior #x10
                                                      (+ (ash col-start -4) x-offset)))
        (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 2)
        ; Low
        (setf (cffi:mem-aref *oled-wbuf* :uint8 0) #x00)
        (setf (cffi:mem-aref *oled-wbuf* :uint8 1) (logand #x0f col-start))
        (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-wbuf* 2))
    ; PIXELS
    (setf (cffi:mem-aref *oled-pixelbuf* :uint8 0) #x40)
    (loop for i from 0 below SSD1306_WIDTH do
        (setf (cffi:mem-aref *oled-pixelbuf* :uint8 (+ 1 i)) (aref *oled-pixels* page i)))
    (i2c-write *dev* SSD1306_SLAVE_ADDR *oled-pixelbuf* (+ 1 SSD1306_WIDTH)))
